...
from text_utils import build_function_inspector_markdown
...
class Gui(Observer):
    def __init__(self, server: ViserServer, state: State, scene: Scene):
        ...
        # This attribute will be assigned externally (from main.py)
        self.camera_params_list = None
        ...
        with self.tab_group.add_tab("Animation"):
            ...
            self.improve_btn = self.api.add_button(label="⇪ Improve Current", disabled=True)
            self.improve_btn.on_click(lambda _: self._open_improve_menu())
            self.details_btn = self.api.add_button("Details", icon=viser.Icon.LIST_DETAILS, disabled=True)
            self.details_btn.on_click(lambda _: self._open_details())
            self.examples_btn = self.api.add_button("Examples", icon=viser.Icon.FOLDER_OPEN)
            self.examples_btn.on_click(lambda _: self._open_examples())
            self.fps_btn_grp = self.api.add_button_group(f"FPS ({self.state.fps})", ["8", "24", "50"])
            self.fps_btn_grp.on_click(lambda _: self._sync_fps())
            self.speed_btn_grp = self.api.add_button_group(label=f"Speed ({self.state.speed})", options=[".25x", ".5x", "1x", "2x"])
            self.speed_btn_grp.on_click(lambda _: self._sync_speed())
            self.frame_slider = self.api.add_slider(label="Frame", min=0, max=self.state.total_frames - 1, step=1, initial_value=0)
            self.frame_slider.on_update(lambda _: self._sync_frame())
            self.play_btn = server.gui.add_button("Play", color="green", icon=viser.Icon.PLAYER_PLAY)
            self.play_btn.on_click(lambda _: self._start_playback())
            self.stop_btn = self.api.add_button("Stop", color="red", visible=False, icon=viser.Icon.PLAYER_STOP)
            self.stop_btn.on_click(lambda _: self._stop_playback())
        ## Scene Tab
        with self.tab_group.add_tab("Scene"):
            self.world_axis_cbox = self.api.add_checkbox("World Axis", initial_value=False)
            @self.world_axis_cbox.on_update
            def _(_):
                self.scene.api.world_axes.visible = self.world_axis_cbox.value
            self.background_cbox = self.api.add_checkbox("Background", initial_value=True)
            ...
            with self.api.add_folder("Example Scenes"):
                ...
                self.api.add_button("Bulldozer").on_click(lambda _: self.state.load_bulldozer())
        ...
        with self.tab_group.add_tab("Render"):
            ...
            # NEW: Re-render Cameras button
            self.rerender_cameras_btn = self.api.add_button("Re-render Cameras", icon=viser.Icon.REFRESH)
            self.rerender_cameras_btn.on_click(lambda event: self._rerender_cameras(event))
    ...
    def _open_generator(self):
        with self.api.add_modal(title="♻ New Animation") as popout:
            ...
            duration_slider = self.api.add_slider(label="Duration", min=1, max=10, step=1, initial_value=1)
            with self.api.add_folder("🔥"):
                design_temperature_slider = self.api.add_slider(label="Design Temperature", min=0.00, max=2.00, step=0.01, initial_value=1.0)
                coding_temperature_slider = self.api.add_slider(label="Code Temperature", min=0.00, max=2.00, step=0.01, initial_value=1.0)
            with self.api.add_folder("👁️"):
                vision_angles_dropdown = self.api.add_dropdown("Vision", options=["none", "front-left", "front + back + sides", "all"], initial_value="none")
            ...
            def _(event: viser.GuiEvent) -> None:
                ...
                generator_config = GeneratorConfig(
                    ...
                    vision_angles=self._vision_angles_from_dropdown(vision_angles_dropdown.value),
    ...
    def _open_details(self):
        ...
        with self.api.add_modal("") as popout:
            with self.api.add_folder("Animation Evolution"):
                for idx, animation in enumerate(self.state.animation_evolution.auto_sampled_animations):
                    self.api.add_button(f"Sample {idx+1}").on_click(lambda _, anim=animation: change_and_close(anim, popout))
                for idx, animation in enumerate(self.state.animation_evolution.auto_improved_animations):
                    self.api.add_button(f"Auto-Improve {idx+1}").on_click(lambda _, anim=animation: change_and_close(anim, popout))
                for idx, animation in enumerate(self.state.animation_evolution.feedback_to_animation.values()):
                    self.api.add_button(f"Feedback-Improve {idx+1}").on_click(lambda _, anim=animation: change_and_close(anim, popout))
            functions_btn = self.api.add_button("Active Functions", icon=viser.Icon.FUNCTION)
    ...
    def _open_examples(self) -> None:
        with self.api.add_modal("") as popout:
            explosion_btn = self.api.add_button("💥 Example Explosion (1s)")
            @explosion_btn.on_click
            def _(_) -> None:
                popout.close()
                from examples import EXAMPLE_EXPLOSION
                self.state.active_animation = EXAMPLE_EXPLOSION
            lsd_btn = self.api.add_button("💮 Example LSD (1s)")
            @lsd_btn.on_click
            def _(_) -> None:
                popout.close()
                from examples import EXAMPLE_LSD
                self.state.active_animation = EXAMPLE_LSD
            acceleration_btn = self.api.add_button("🚕 Example Acceleration (2s)")
            @acceleration_btn.on_click
            def _(_) -> None:
                popout.close()
                from examples import EXAMPLE_ACCELERATION
                self.state.active_animation = EXAMPLE_ACCELERATION
            breathing_btn = self.api.add_button("🌬️ Example Breathing (3s)")
            @breathing_btn.on_click
            def _(_) -> None:
                popout.close()
                from examples import EXAMPLE_BREATHING
                self.state.active_animation = EXAMPLE_BREATHING
            color_shift_btn = self.api.add_button("🌈 Example Color Shift (4s)")
            @color_shift_btn.on_click
            def _(_) -> None:
                popout.close()
                from examples import EXAMPLE_COLOR_SHIFT
                self.state.active_animation = EXAMPLE_COLOR_SHIFT
            lava_melting_btn = self.api.add_button("🌋 Example Lava Melting (5s)")
            @lava_melting_btn.on_click
            def _(_) -> None:
                popout.close()
                from examples import EXAMPLE_LAVA_MELTING
    ...
    def _render_semisphere(self, event: viser.GuiEvent):
        ...
        assert client is not None
        from renderer_semisphere import RendererSemisphere
        renderer = RendererSemisphere(client, self.api, self.state, self.server, self.camera_params_list)
        renderer.render_animation()


    def _rerender_cameras(self, event: viser.GuiEvent) -> None:
        # This is triggered when the "Re-render Cameras" button is pressed.
        if hasattr(self, 'camera_params_list') and self.camera_params_list is not None:
            ...
            # event.client.add_notification("Re-render", "Camera views updated", auto_close=True, color="green")
        else:
            print('error')
            # event.client.add_notification("Error", "Camera parameters not set", auto_close=True, color="red")
    ...
    def _open_active_functions(self):
        ...
        markdown = build_function_inspector_markdown(centers_code, rgbs_code, opacities_code)
        with self.api.add_modal("") as popout:
    ...
    def _vision_angles_from_dropdown(self, dropdown_value: str) -> list[VisionAngle]:
        match dropdown_value:
            ...
            case "all":
                return ["front", "front-left", "left", "back-left", "back", "back-right", "right", "front-right"]
            case _:
                return []
