...
class Gui(Observer):
    def __init__(self, server: ViserServer, state: State):
        ...
        self.animation_md = self.gui_api.add_markdown("Animation: **None**")
    ...
    def load_splats(self) -> None:
        ...
        for frame in range(self.state.fps):
    ...
    def open_generator(self):
        with self.gui_api.add_modal(title="🤖 Animation Generator") as popout:
            name_txt = self.gui_api.add_text("Name", self.state.animation_name)
            input_txt = self.gui_api.add_text("Prompt", self.state.animation_prompt)
            ...
            def _(_) -> None:
                if not input_txt.value:
                    return
                self.state.animation_name = name_txt.value
                self.state.animation_prompt = input_txt.value
                self.state.temperature = temperatur_slider.value
                popout.close()
                centers_fn_code, rgbs_fn_code, opacities_fn_code = self.generate_functions(prompt=input_txt.value, temperature=temperatur_slider.value)
            ...
            abort_btn = self.gui_api.add_button("Abort", icon=viser.Icon.X, color="red")
            @abort_btn.on_click
            def _(_) -> None:
                popout.close()
    ...
    def open_feedback(self):
        with self.gui_api.add_modal(title="🤖 Feedback Loop") as popout:
            ...
            improve_btn = self.gui_api.add_button("🛠 Generate")
    ...
    def open_prompt(self):
        with self.gui_api.add_modal(title="") as popout:
            generator_btn = self.gui_api.add_button(label="New Animation")
            @generator_btn.on_click
            def _(_) -> None:
                popout.close()
                self.open_generator()

            feedback_btn = self.gui_api.add_button(
                label="Improve Current",
                color="yellow",
                disabled=not self.state.has_animation)
    ...
    def play_animation(self):
        ...
        while True:
            for frame in range(self.frame_slider.value+1, self.state.fps):
    ...
    def generate_functions(self, prompt: str, temperature: float):
    ...
    def generate_functions_with_feedback(self, feedback: str):
        ...
        raw_centers_function = prompt_llm(prompt=feedback, system_message=centers_system_msg, temperature=self.state.temperature)
        progress.value = 15
        rgbs_system_msg = prompts.RGBS_FEEDBACK.format(context=self.state.rgbs_context)
        raw_rgbs_function = prompt_llm(prompt=feedback, system_message=rgbs_system_msg, temperature=self.state.temperature)
        progress.value = 30
        opacities_system_msg = prompts.OPACITIES_FEEDBACK.format(context=self.state.opacities_context)
        raw_opacities_function = prompt_llm(prompt=feedback, system_message=opacities_system_msg, temperature=self.state.temperature)
        progress.value = 45

        status.content = "*Validating Python...*"
        validated_centers_function = prompt_llm(prompt=raw_centers_function, system_message=prompts.PYTHON_VALIDATION, temperature=self.state.temperature)
        progress.value = 60
        validated_rgbs_function = prompt_llm(prompt=raw_rgbs_function, system_message=prompts.PYTHON_VALIDATION, temperature=self.state.temperature)
        progress.value = 75
        validated_opacities_function = prompt_llm(prompt=raw_opacities_function, system_message=prompts.PYTHON_VALIDATION, temperature=self.state.temperature)
        progress.value = 90
